...
const register = async (req, res) => {
  try {
    ...
    // Generar token utilizando valores por defecto si las variables de entorno no están definidas  
    ...
    res.status(201).json({
      status: 'success',
      data: { token, user: { id: userId, email } }
    });
  } catch (error) {
...
const login = async (req, res) => {
  try {
    ...
    // Generar token utilizando fallback en caso de que las variables de entorno no se definan  
    const token = jwt.sign(
